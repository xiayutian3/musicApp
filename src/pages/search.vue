...
<div class="search">
  <div class="search-title">
     ...
     <button type="button" @click="searchHandler(searchContent)" name="button">搜索</button>
  </div>
  <ul class="list searchlist">
     <router-link tag="li" class="song" :to="{name:'MusicPlay',params:{songid:item.songid}}" v-for="(item,i) in songlist" :key="i" >
       <div class="left">
...
export default {
  ...
  methods:{
    searchHandler(searchContent){
      request.getSongs({query:searchContent},res=>{
        console.log(11111)
